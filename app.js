...
app.post('/restaurants/add', async (req, res) => {
  const { name, email, location, description, category, phone, openingTime, closingTime, menuUrl, locationUrl, logoUrl, photoUrl, status } = req.body;
  ...
  try {
    ...
    res.redirect('/'); // Redirect back to the main page after adding the restaurant
  } catch (err) {
